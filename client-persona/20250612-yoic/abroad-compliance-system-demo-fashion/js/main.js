...
function addLoadingAnimations() {
  const elements = document.querySelectorAll('.hero-section, .content-section, .file-card');
...
function generateFileCards() {
  ...
  // 图标映射
  const iconMap = {
    '制度文件': 'fas fa-file-contract',
    '培训资料': 'fas fa-graduation-cap',
    '风险评估报告': 'fas fa-chart-line',
    '会议纪要': 'fas fa-users',
    '工作计划': 'fas fa-tasks',
    '调研报告': 'fas fa-search'
  };
  ...
  documentStructure.forEach(category => {
    category.children.forEach(subcategory => {
      subcategory.documents.forEach(doc => {
        const iconClass = iconMap[subcategory.name] || 'fas fa-file-alt';
        cardsHTML += `
          <div class="file-card">
            <div class="file-icon">
              <i class="${iconClass}"></i>
            ...
            <div class="file-info">
              ...
              <div class="file-meta">
                <span class="file-type">${subcategory.name}</span>
                <a href="#" class="view-document" data-path="${doc.path}">
